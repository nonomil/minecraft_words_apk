...
> 扫描时间: 2026-03-02T00:32
> 数据来源: `docs\scan\2026-03-01-full-codebase-scan\scan.db`

### 入口点
- `batch_merge()` (batch_merger.py:20) → 未记录
- `merge_images()` (merger.py:10) → 未记录
- `scan()` (scan.py:100) → 未记录

### 核心模块
- `mod:.claude/scripts`: Git 钩子，自动化脚本
- `mod:.claude/skills`: 技能定义，模板文件
- `mod:.claude/workflows`: 工作流定义，路由规则
- `mod:image-merger/src`: Python 图片合并应用

### 关键约束
- [claude-workflow-constants.md] Codex 三必填参数
- [claude-workflow-constants.md] 单次 diff ≤ 200 行
- [CODEBASE_MAP.md] 禁止 rm -rf / git reset --hard

### 高风险变更点
- `claude-workflow-constants.md` (风险: high)
- `merger.py:merge_images()` (风险: high)
