...
function generateSpellingTask(word){
  ...
  if (spellingSubmode === 'spell') {
    ...
    // 新增：提交按钮后面的“单词提示框”，默认隐藏，由标题栏“提示”按钮控制
    const hintBox = document.createElement('div');
    hintBox.id = 'spellingHintBelow';
    hintBox.className = 'spelling-inline-hint';
    hintBox.textContent = original; // 直接显示标准化英文
  ...
  // 新增：提交答案后面的“单词提示框”，默认隐藏
  const hintBox = document.createElement('div');
  hintBox.id = 'spellingHintBelow';
  hintBox.className = 'spelling-inline-hint';
  hintBox.textContent = original;
  hintBox.style.display = 'none';
  controls.appendChild(hintBox);
...
function updateQuizDisplay() {
  ...
  try {
    ...
    if (headline) {
      // 移除旧的头部内联提示
      const oldInline = headline.querySelector('#spellingInlineHint');
      ...
      if (!toggle) {
        ...
        btnSpell.type = 'button'; btnSpell.className = 'spelling-submode-btn spell'; btnSpell.textContent = '拼写';
        btnSpell.onclick = ()=> setSpellingSubmode && setSpellingSubmode('spell');
        const btnFill = document.createElement('button');
        btnFill.type = 'button'; btnFill.className = 'spelling-submode-btn fill'; btnFill.textContent = '补全';
      ...
      if (playBtn && playBtn.parentNode) {
        if (toggle.parentNode !== headline) playBtn.insertAdjacentElement('afterend', toggle); else { playBtn.insertAdjacentElement('afterend', toggle); }
      } else {
        if (toggle.parentNode !== headline) headline.appendChild(toggle);
      }

      // 新增：在“补全”后面加“提示”按钮（点击后在下方提交按钮后显示提示框）
      let hintTrigger = headline.querySelector('#spellingHintTrigger');
      if (!hintTrigger) {
        hintTrigger = document.createElement('button');
        hintTrigger.id = 'spellingHintTrigger';
        hintTrigger.className = 'control-btn hint';
        hintTrigger.textContent = '提示';
      }
      if (toggle.parentNode === headline) {
        toggle.insertAdjacentElement('afterend', hintTrigger);
      } else {
        headline.appendChild(hintTrigger);
      }
      hintTrigger.onclick = () => {
        const en = (word.standardized || word.word || '').trim();
        const box = document.getElementById('spellingHintBelow');
        if (box) {
          box.textContent = en || '';
          box.style.display = (box.style.display === 'none' || !box.style.display) ? 'inline-block' : 'none';
