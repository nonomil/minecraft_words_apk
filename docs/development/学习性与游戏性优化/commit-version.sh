...
if [ -z "$VERSION" ]; then
    echo "❌ 错误：请指定版本号"
    echo "使用方法：./commit-version.sh v1.6.0"
    exit 1
fi
...
case $VERSION in
    v1.6.0)
        TITLE="学习系统 - 答题数据结构扩展"
        BODY="- 新增 challengeStats 数据结构记录答题统计
...
- 设置面板增加环境单词开关"
        ;;
    v1.6.3)
        TITLE="学习系统 - Biome 切换复习"
        BODY="- Biome 切换时触发快速复习（3题）
- 基于间隔重复算法选择复习单词
- 优先复习错误率高和久未复习的单词
- 全对奖励：+90分 +1💎
